...
在 Minecraft **主菜单**和**游戏中**播放你自己的背景音乐，支持多种音频格式。

> 原版音乐只在特定场景出现？**主菜单就能播放你喜欢的音乐！**
...
> **无 FFmpeg 版**（`noffmpeg-universal.jar`）只能播放 `.ogg` `.wav` `.mp3` `.aiff` `.aif` `.au`。
...
/music shufflemode WEIGHTED        → 收藏歌曲权重 x3
